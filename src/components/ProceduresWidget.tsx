...
import { ArrowUp, ClipboardCheck, Loader2 } from "lucide-react";
...
const EVENTS: Evt[] = [
  ...
  { delay: 1800, op: "replace", msg: { id: 1, kind: "ai", lines: [
    "Jutro załadunek do UK (TIR-248) · Wymagana plomba celna na naczepie.",
    "Brak potwierdzenia od kierowcy.",
  ]}},
  { delay: 400,  op: "add",     msg: { id: 2, kind: "cta",      text: "Sprawdź procedurę" } },
  { delay: 1000, op: "add",     msg: { id: 3, kind: "ai",       lines: ["Czy wysłać kierowcy checklistę celną przed załadunkiem?"] } },
  { delay: 1400, op: "add",     msg: { id: 4, kind: "user",     text: "Tak, wyślij checklistę" } },
  { delay: 700,  op: "add",     msg: { id: 5, kind: "progress", text: "Wysyłam checklistę celną do kierowcy..." } },
  { delay: 3000, op: "reset" },
];

export function ProceduresWidget() {
            ...
            if (msg.kind === "typing") {
              return (
                <div key={msg.id} className="chat-msg flex items-center gap-2 px-1">
                  ...
                  <p className="text-[13px] text-[#9333ea] tracking-[0.4px]">Sprawdzam procedury...</p>
                </div>
              );
            }
          })}
