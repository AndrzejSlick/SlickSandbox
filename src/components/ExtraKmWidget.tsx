...
const EVENTS: Evt[] = [
  ...
  { delay: 1800, op: "replace", msg: { id: 1, kind: "ai", lines: [
    "TIR-192 · Odchylenie od trasy: +47 km.",
    "Kierowca jedzie przez Łódź zamiast A1.",
    "Szacowany koszt objazdu: ok. 14 €.",
  ]}},
  { delay: 400,  op: "add",     msg: { id: 2, kind: "cta",      text: "Pokaż trasę" } },
  { delay: 1000, op: "add",     msg: { id: 3, kind: "ai",       lines: ["Czy zapytać kierowcę o powód objazdu?"] } },
  { delay: 1400, op: "add",     msg: { id: 4, kind: "user",     text: "Tak, zapytaj kierowcę" } },
  { delay: 700,  op: "add",     msg: { id: 5, kind: "progress", text: "Wysyłam wiadomość do kierowcy..." } },
  { delay: 3000, op: "reset" },
];

export function ExtraKmWidget() {
            ...
            if (msg.kind === "typing") {
              return (
                <div key={msg.id} className="chat-msg flex items-center gap-2 px-1">
                  ...
                  <p className="text-[13px] text-[#9333ea] tracking-[0.4px]">Analizuję trasę...</p>
                </div>
              );
            }
          })}
